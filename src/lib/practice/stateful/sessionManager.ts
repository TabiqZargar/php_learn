 ...
 *     academy_db_config.php  (mysql sessions) leaner-visible connection config
 *                            with the session-scoped credentials + table prefix
 *
...
import {
  cleanupMysqlSessionTables,
  resolveMysqlRuntime,
  seedMysqlSessionTables,
} from "../mysql/runtime.ts";
import {
  MysqlRuntimeUnavailableError,
  type MysqlSessionConfig,
} from "../mysql/runtime.ts";
import { phpStringLiteral } from "../mysql/config.ts";
...
/** Options controlling how a practice session is provisioned. */
export interface CreateStatefulSessionOptions {
  /** Execution capability of the program. "mysql" sessions get database access. */
  capability?: ExecutionCapability;
  /** MySQL-specific provisioning (ignored unless capability === "mysql"). */
  mysql?: {
    /** Logical table names to create + seed into this session's namespace. */
    seedTables?: string[];
  };
}

/**
 * Create an isolated session and its throwaway workspace. For mysql sessions
 * this also resolves the configured practice database, probes it, generates a
 * random per-session table prefix, writes the learner-visible connection
 * config file into the workspace and seeds the requested tables. It throws a
 * MysqlRuntimeUnavailableError when MySQL practice cannot be used so callers
 * can surface a clear runtime_unavailable instead of a wrong answer.
 */
export async function createStatefulSession(
  programSlug: string,
  options?: CreateStatefulSessionOptions,
): Promise<StatefulPracticeSession> {
  const capability = options?.capability;
  let mysql: MysqlSessionConfig | undefined;
  if (capability === "mysql") {
    const runtime = await resolveMysqlRuntime();
    if (!runtime.ok) {
      throw new MysqlRuntimeUnavailableError(runtime.reason);
    }
    mysql = {
      ...
      tablePrefix: `s${randomBytes(6).toString("hex")}_`,
    };
  }
  ...
  try {
    ...
    if (mysql) {
      await writeFile(
        join(workspacePath, "academy_db_config.php"),
        buildAcademyDbConfigSource(mysql),
        "utf8",
      );
    }
    if (mysql && options?.mysql?.seedTables && options.mysql.seedTables.length > 0) {
      const seeded = await seedMysqlSessionTables(mysql, options.mysql.seedTables);
      if (!seeded.ok) {
        throw new MysqlRuntimeUnavailableError(seeded.reason);
      }
    }
  } catch (error) {
    await rm(workspacePath, { recursive: true, force: true });
    if (mysql) {
      void cleanupMysqlSessionTables(mysql);
    }
    throw error;
  }
...
/**
 * PHP source of the learner-visible connection config injected into a mysql
 * session workspace. The values come straight from the validated runtime
 * config and are single-quote escaped so nothing can break out of the file.
 * Exported for direct unit testing.
 */
export function buildAcademyDbConfigSource(config: MysqlSessionConfig): string {
  return `<?php
// Generated by the PHP Academy runner for this practice session.
// Treat these values as secrets belonging to this session only.
return array(
  "host" => ${phpStringLiteral(config.host)},
  "port" => ${config.port},
  "database" => ${phpStringLiteral(config.database)},
  "user" => ${phpStringLiteral(config.user)},
  "password" => ${phpStringLiteral(config.password)},
  "table_prefix" => ${phpStringLiteral(config.tablePrefix)},
);
`;
}
...
export async function destroyStatefulSession(id: string): Promise<boolean> {
  ...
  // MySQL sessions leave physical tables behind in the practice database;
  // drop them (best-effort — failures never block or reject).
  if (record.session.mysql) {
    await cleanupMysqlSessionTables(record.session.mysql);
  }
